...
pub async fn start_full_internet_research_session(
    ...
    // DISABLED: Web scraping temporarily disabled to prevent low-quality facts from being
    // added to the knowledge base. Will be re-enabled with proper content curation.
    log_research_event(db, "Web research disabled - returning without scraping", run_id).await;
    Ok(())
}
