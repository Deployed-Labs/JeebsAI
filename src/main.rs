...
async fn main() -> std::io::Result<()> {
	...
	.bind(("0.0.0.0", std::env::var("PORT").unwrap_or_else(|_| "8080".to_string()).parse::<u16>().unwrap_or(8080)))?;            .service(auth::change_password)
...
.bind(("0.0.0.0", std::env::var("PORT").unwrap_or_else(|_| "8080".to_string()).parse::<u16>().unwrap_or(8080)))?;
    ...
    let cli_state = web::Data::new(AppState {
        db: db.clone(),
        plugins: cli_plugins,
