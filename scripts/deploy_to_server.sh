...
cp packaging/jeebs.service "$TMPDIR/jeebs-deploy/"
cp packaging/jeebs.env.example "$TMPDIR/jeebs-deploy/"
...
if [ -n "$SSH_PASS" ]; then
  ...
  sshpass -p "$SSH_PASS" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "${SSH_OPTS[@]}" "$TMPDIR/jeebs-deploy.tar.gz" "${DEST}:/tmp/jeebs-deploy.tar.gz"
  echo "Installing on remote host..."
  sshpass -p "$SSH_PASS" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "${SSH_OPTS[@]}" "${DEST}" \
...
REMOTE
  ...
  scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "${SSH_OPTS[@]}" "$TMPDIR/jeebs-deploy.tar.gz" "${DEST}:/tmp/jeebs-deploy.tar.gz"
  echo "Installing on remote host..."
  ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "${SSH_OPTS[@]}" "${DEST}" \
    REMOTE_DIR="$REMOTE_DIR" bash -s <<'REMOTE'
set -euo pipefail
sudo mkdir -p "$REMOTE_DIR"
sudo tar -C "$REMOTE_DIR" -xzf /tmp/jeebs-deploy.tar.gz
